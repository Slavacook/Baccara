...
const SLOT_COUNT_SMALL = 6    # Режим с 6 слотами (масштаб 1.0)
const SLOT_COUNT_LARGE = 10   # Режим с 10 слотами (масштаб 0.6)
const SCALE_SMALL = 1.0       # Масштаб для 6 слотов
const SCALE_LARGE = 0.6       # Масштаб для 10 слотов
...
var slot_count: int = SLOT_COUNT_SMALL  # Текущее количество слотов (6 или 10)
var current_scale: float = SCALE_SMALL  # Текущий масштаб стопок
...
func _init(container: Control):
	...
	_initialize_slots(SLOT_COUNT_SMALL)
...
func _initialize_slots(count: int):
	...
	slot_count = count
	current_scale = SCALE_SMALL if count == SLOT_COUNT_SMALL else SCALE_LARGE
...
func add_chip(denomination: float) -> bool:
	...
	if target_stack == null:
		...
		if chip_stacks.size() >= slot_count:
			...
			if slot_count == SLOT_COUNT_SMALL:
				_rescale_to_large()
			else:
	...
	if target_stack.add_chip():
		...
		push_warning("ChipStackManager: Стопка полна! (макс %d фишек)" % ChipStack.MAX_CHIPS)
		return false
...
func _rescale_to_large():
	...
	_initialize_slots(SLOT_COUNT_LARGE)
...
func _compact_stacks():
	...
	if chip_stacks.size() <= SLOT_COUNT_SMALL and slot_count == SLOT_COUNT_LARGE:
		_rescale_to_small()
